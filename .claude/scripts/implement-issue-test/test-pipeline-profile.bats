...
#
# minimal profile: always returns 2 regardless of config value (floor of 2
#   guarantees at least one fix is applied and re-reviewed before a block)
# standard profile: passes config value through unchanged
# full profile: passes config value through unchanged
# =============================================================================

@test "pr review: minimal profile floors max_iter at 2 (config says 1)" {
	local result
	result=$(apply_profile_to_pr_review_max_iter "minimal" "1")
	[[ "$result" == "2" ]]
}

@test "pr review: minimal profile floors max_iter at 2 (config says 2)" {
	local result
