...
# The COMPLETE stage lives inline in main(), but the appendix logic itself is
# extracted into the standalone append_reconciled_tally() helper (issue #914
# task 1), sourced by source_orchestrator_functions() in setup() above — so
# these tests call the real orchestrator function directly instead of a
# copied inline reimplementation, and no drift-guard test is needed to keep
# a mirror in sync with it.
# =============================================================================
...
@test "completion summary: reconciled_count of 0 leaves the summary unchanged" {
	...
	local result
	result=$(append_reconciled_tally "Implementation completed successfully")
